...
void Editor::call(QString bin) {
    ...
    QProcess p(0);
    //    根据bin的值调用相应的命令
    p.execute(tr("echo"),  QStringList() << "file path should not contain Chinese characters.");
    if (bin == tr("scan")) {
        ...
        //if (QFileInfo::exists(fileName.replace(QRegExp("\\.mc"), "\.tm"))) {
        ...
        ui->outputArea->setText(QString::fromLocal8Bit("本程序暂时不支持从gui上运行虚拟机，请使用命令行操作"));
        return;
    }
    //   开始执行
    p.waitForStarted();
    ...
    if (!err.isEmpty()) {
       // ui->outputArea->setText(tr("Error happended\n")+err);
    ...
    //if (bin == tr("genCode")) {
    //    ui->outputArea->setText(QString::fromLocal8Bit("tm文件已保存到目录:")+fileName.replace(QRegExp("\\.mc"), "\.tm"));
...
//    加载文本
    QTextStream in(&f);
    ui->codeArea->clear();
    QString codeTxt = in.readAll();
    ui->codeArea->setPlainText(codeTxt.toLocal8Bit());
    f.close();
}
...
void Editor::saveOutput()
{
    ...
    if (f.open(QIODevice::Text | QIODevice::ReadWrite)) {
        ...
        f.close();
        ui->statusbar->showMessage(QString::fromLocal8Bit("输出结果保存成功"), 3);
    }
    else {
        ui->statusbar->showMessage(QString::fromLocal8Bit("输出结果保存失败"), 3);
    }
}
