...
var_declaration: type_specifier ID SEMI
				...
				| type_specifier ID LB NUM 
				{
					// 如果有NUM应立刻用一个变量暂存因为后面的识别会把TokenValue覆盖掉
					$<node>$ = (TreeNode *) newDeclNode(ArrVarK);
					$<node>$->child[0] = $1;
					$<node>$->lineno = lineno;
...
fun_declaration: type_specifier ID 
				{
					$<node>$ = (TreeNode *) newDeclNode(FuncK);
					$<node>$->lineno = lineno;
					strcpy($<node>$->attr.name, $2);
				}
				LP params RP compound_stmt
				{
					$$ = $<node>3;
...
param: type_specifier ID 
		...
		| type_specifier ID
		{
			$<node>$ = (TreeNode *) newParamNode(ArrParamK);
			$<node>$->child[0] = $1;
			strcpy($<node>$->attr.name, $2); // TODO 修改 数组
		}
		LB RB { $$ = $<node>3; }  
		;
...
var: ID 
	...
	| ID 
	{
		$<node>$ = (TreeNode *) newExpNode(ArrIdK);
		strcpy($<node>$->attr.name, $1); // TODO 修改 数组
	}
	LB expression RB
	{
		$$ = $<node>2;
